...
int main()
{
	...
	NmppsFrame_8u srcFrame,dstFrame;
	unsigned char* srcImg8 = nmppsMallocFrame_8u(size,width,&srcFrame);	// Allocate source image buffer with guard fields of two rows
	unsigned char* dstImg8 = nmppsMallocFrame_8u(size,width,&dstFrame);	// Allocate result image buffer 
	...
	nmppsFreeFrame(&srcFrame);	// Free source image buffer 
	nmppsFreeFrame(&dstFrame);		  	// Free result image buffer 
    return 0;
}
