...
int main()
{
	...
	int ok;
	ok=Connector.Sync(width);		// Barrier sync/ Send width to nmc
	ok=Connector.Sync(height);		// Barrier sync/ Send height to nmc
	ok=Connector.Sync(0);			// Barrier sync/ Get status of memory allocation from nm
	if (ok!=0x600DB00F){
