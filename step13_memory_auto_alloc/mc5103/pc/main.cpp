...
int main()
{
	...
	//if(!VS_Bind("../../../../input/Lena256x240.bmp"))
	...
	while(VS_Run())
	{
		...
		//VS_Text("%7d clocks per frame, %.2f clocks per pixel, %.2f fps route=%08x-%08x\r\n", t, 1.0*t/size, 320000000.0/t ,routeh,route);
		//VS_Text("%7d clocks per frame, %.2f clocks per pixel, %.2f fps route=%08x-%08x\r\n", t, 1.0*t/size, 320000000.0/t ,routeh,route);
		//for(int i=routeSize-1; i>=0; i--)
		//	VS_Text("%08x.", route[i]);
		VS_Text("    %7d clocks per frame, %.2f clocks per pixel, %.2f fps \r\n", t, 1.0*t/size, 320000000.0/t);
