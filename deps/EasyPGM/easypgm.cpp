...
int save_pgm_header(void* header, int width, int height, char* copyright)
{
	...
	nmppsPut_8u(addr,pos++,'P'); 
	nmppsPut_8u(addr,pos++,'5'); 
	nmppsPut_8u(addr,pos++,0x0A);
	nmppsPut_8u(addr,pos++,'#');
	...
	for(int i=0; i<copyright_len;i++ )
		nmppsPut_8u(addr,pos++,copyright[i]); 
	...
	for (int i=0; i<width_len; i++) 
		nmppsPut_8u(addr,pos++,width_str[i]); 
	nmppsPut_8u(addr,pos++,' ');

	// put  height into header
	for (int i=0; i<height_len; i++) 
		nmppsPut_8u(addr,pos++,height_str[i]); 
	nmppsPut_8u(addr,pos++,0x0A);
	
	// put max gray into header
	nmppsPut_8u(addr,pos++,'2');
	nmppsPut_8u(addr,pos++,'5');
	nmppsPut_8u(addr,pos++,'5');
	nmppsPut_8u(addr,pos++,0x0A);
...
int read_pgm_header(void* header, int& width, int& height){
	...
	symbol=nmppsGet_8u(addr,pos++); if (symbol!='P')  return 0;
