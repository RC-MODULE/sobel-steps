...
int main()
{
	...
	ok=halSync(0);												// Barrier sync before call of Sobel filter on NMC

	//... wait while sobel is runing on board
	ok = halSync(777);			// Barrier sync. Wait until Sobel filter is finished 
	if (ok == 0xBEDA)
		printf("Memory allocation error!\n");
	unsigned t=halSync(0);									// Recieve Soblel processing time 
