...
int main()
{
	...
	int ok;
	ok=halSync(width);		// Barrier sync/ Send width to nmc
	ok=halSync(height);		// Barrier sync/ Send height to nmc
	ok=halSync(0);			// Barrier sync/ Get status of memory allocation from nm
	if (ok!=0x600DB00F){
