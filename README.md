...
Хотя некоторые пакеты SDK могут изначально присутствовать у пользователя, для обеспечения совместимости версий и упрощения процедуры настройкипутей все зависимые компоненты SDK (nmsdk, по поддержки плат) рекомендуется их установить из  пакета https://github.com/RC-MODULE/dev-kit. 
...
* Запуск автономного (standalone) приложения  осуществляется также командой 
```
make run
```
