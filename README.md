...
Руководство предназначено для ознакомления с программированием процессоров семейства NeuroMatrix (NM6405,NM6406), работой с инструментальными платами МВ77.07 и МС5103.
...
- hal [платформенно-независимая коммуникационная библиотека для работы с платами](https://github.com/RC-MODULE/hal)
...
Настройка переменных и зависимостей производится в файле */sobel-steps/global.mk* и при необходимости могут быть переопределены. 
...
- двух-компонентные приложения *host-target*, где загрузка и управление NMC-программой на плате
 осуществляется со стороны хост-программы, разработанной на С++ в среде Microsoft Visual Studio (2005 или 2015)
- x86 прототип приложения *pc-standalone*, самостоятельное x86-приложение функционально  полностью эмулирующее работу nmc-приложения.
...
При необходимости генерация сборочных  файлов осуществляется командой 
```\sobel-steps\step01_easybmp_risc_8bit> make configure ``` из соответствующей папки с примером.

Сборка проектов под VisualStuduio 2005 или 2015 соответственно командами  генерации сборочных  файлов осуществляются командами   
...
Для досупа приложений к плате MB77.07 используется переменная окружения ``` MB7707MAC ``` , содрежащая MAC-адрес сетевого адаптера, подключенного к плате MB77.07.


 * Данную переменную необходимо определить либо глобально в окружении ОС Windows, либо локально  в **/sobel-steps/global.mk** .  По умолчанию MB7707MAC определяется  в **/sobel-steps/global.mk** как:   
``` MB7707MAC ?= 1A-2B-3C-4D-5E-6F ```  

 *  В host-target приложении MAC адрес задается в **main.cpp**
```
   unsigned char MAC_ADDRESS[] = {0x1a, 0x2b, 0x3c, 0x4d, 0x5e, 0x6f}
```

Значение переменной MB7707MAC (MAC_ADDRESS)  необходимо исправить на соответствующий МАС-адрес сетевого хост-адаптера, либо переопределить сам адрес сетевого адаптера. 

> Узнать или переопределить MAC адрес сетевой карты можно с помощью программы [Technitium MAC Address Changer](https://technitium.com/tmac/)

> Необходимо убедиться, что перемычка на плате MB77.07 установлена (Загрузка OS Linux - не производится).

> Также должен быть установлен [WinPCAP](https://www.winpcap.org/install/) 
...
#### Шаг 1. [Портирование С++ кода на NMC RISC-ядро с эмуляцией байтового доступа](/step01_easybmp_port2nmc_8bit/)
